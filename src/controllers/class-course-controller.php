...
class Course_Controller extends Page_Controller {
	...
	public static function single( $default_template = null ) {
		...
		if ( static::$post_type === $post->post_type ) {
			...
			$fields['use_autoevaluation_form']  = is_plugin_active( 'bf2-gravityforms/bf2-gravityforms.php' ) ? get_post_meta( $fields['badge_page']->ID, 'autoevaluation_form_type', true ) !== 'none' : false;
			$fields['autoevaluation_form']      = is_plugin_active( 'bf2-gravityforms/bf2-gravityforms.php' ) && $fields['use_autoevaluation_form'] ? get_post_meta( $fields['badge_page']->ID, 'autoevaluation_form_id', true ) : '';
